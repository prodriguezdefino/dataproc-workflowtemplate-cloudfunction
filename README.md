...
```
{
  ...
  "jobs":[                   // a list of dataproc based jobs to be executed, this can be ordered using pre-requisites and the structure is completely based on the Python API for Dataproc jobs request ([check here the REST API version](https://cloud.google.com/dataproc/docs/reference/rest/v1beta2/projects.locations.workflowTemplates#orderedjob the Python version uses snake case for the JSON entries))
    {
      "step_id":"step1",
      "pyspark_job": {
...
```
This request will instantiate a new cluster, execute the PySpark job contained in the location, wait for the completion, destroy the cluster resources and propagate results to the created Pubsub topic for later inspection.
